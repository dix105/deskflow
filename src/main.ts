...
const importWindowsAppsButton = document.querySelector<HTMLButtonElement>('#importWindowsApps')!;
...
aiVoiceCommandsInput.addEventListener('change', () => {
  ...
  setStatus('success', aiVoiceCommandsEnabled ? 'Cerebras command brain enabled.' : 'Cerebras command brain disabled.');
});
...
importWindowsAppsButton.addEventListener('click', () => importWindowsApps());
...
async function importWindowsApps(options: { silent?: boolean } = {}) {
  if (!isTauriRuntime || !isWindowsDesktop) {
    if (!options.silent) setStatus('idle', 'Windows app import only runs in the Windows desktop app.');
    return;
  }
  const discovered = await invoke<CommandTarget[]>('discover_windows_app_targets');
  const before = commandTargets.length;
  ...
  saveCommandTargets();
  renderCommandTargets();
  const imported = commandTargets.length - before;
  addDebugEvent('windows_apps_imported', { discovered: discovered.length, imported });
  if (!options.silent) setStatus('success', imported ? `Imported ${imported} Windows apps into voice commands.` : 'Windows apps are already imported.');
}
...
function shouldUseAiVoiceCommandFallback(phrase: string, localDecision: VoiceCommandDecision) {
  const normalized = phrase.toLowerCase().replace(/[.,!?]/g, ' ').replace(/\s+/g, ' ').trim();
  if (!normalized || normalized.length < 5) return false;
  if (/^(thank you|thanks|see you|i'm sorry|sorry|okay|ok|yes|no|cancel|confirm)\b/.test(normalized)) return false;
  if (!/\b(open|launch|start|close|quit|stop)\b/.test(normalized)) return false;
  return localDecision.action === 'none' || localDecision.target.startsWith('search:');
}

async function classifyVoiceCommandWithCerebras(phrase: string): Promise<VoiceCommandDecision> {
